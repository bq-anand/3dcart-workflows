...
_3DCartSaveCustomers = require "../../../../../../lib/Strategy/DBStrategy/Save/TemporaryTable/_3DCartSaveCustomers"
...
describe "_3DCartSaveCustomers", ->
  dependencies = createDependencies(settings, "_3DCartSaveCustomers")
  ...
  beforeEach ->
    strategy = new _3DCartSaveCustomers(
      _.defaults {}, input
    ,
      dependencies
    )
    ...
    .then ->
      knex.transaction (transaction) =>
        ...
        .then -> strategy.insert _.defaults
          Email: "tbitnernewemail@gmail.com"
        , sample
        .then -> strategy.finish()
    .then ->
      knex(_3DCartCustomers::tableName).count("id")
      .then (results) ->
        results[0].count.should.be.equal("1")
    .then ->
      _3DCartCustomers.where({Email: "tbitnernewemail@gmail.com"}).fetch()
      .then (model) ->
        should.exist(model)
