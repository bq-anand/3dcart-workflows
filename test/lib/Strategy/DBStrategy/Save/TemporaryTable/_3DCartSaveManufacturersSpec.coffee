...
_3DCartSaveManufacturers = require "../../../../../../lib/Strategy/DBStrategy/Save/TemporaryTable/_3DCartSaveManufacturers"
...
describe "_3DCartSaveManufacturers", ->
  dependencies = createDependencies(settings, "_3DCartSaveManufacturers")
  ...
  beforeEach ->
    strategy = new _3DCartSaveManufacturers(
      _.defaults {}, input
    ,
      dependencies
    )
  ...
  it "should update existing objects @fast", ->
    ...
    .then ->
      ...
      .then (results) ->
        results[0].count.should.be.equal("1")
    .then ->
      _3DCartManufacturers.where({ManufacturerName: "New Manufacturer Name"}).fetch()
      .then (model) ->
        should.exist(model)
